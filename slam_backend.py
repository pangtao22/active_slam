...
class SlamBackend:
    def __init__(self, t_xy_e_0, frontend: SlamFrontend):
        ...
        self.dim_X = 3  # dimension of poses.
    ...
    def get_X_WB_initial_guess_array(self):
        ...
        """
        n_o = len(self.odometry_measurements)
        X_WB_e = np.ones((n_o + 1, self.dim_X))
        for i in range(n_o):
            X_WB_e[i] = self.X_WB_e_dict[i]
        if n_o > 0:
    ...
    def calc_jacobian_and_b(self, X_WB_e, l_xy_e):
        ...
        n_rows = n_o * dim_X + nl_measurements * dim_l
        ...
        for i in range(n_o):
            ...
            # bearing
            bi[2] = calc_angle_diff(X_WB_e[i, 2], X_WB_e[i + 1, 2])
        ...
        for i_k, (k, visible_robot_poses) in enumerate(
            ...
            for i in visible_robot_poses.keys():
                ...
                a = 1
                J[i_row, j_start: j_start + 2] += -d_arctan_D_dxy * a
                J[i_row, j_start + 2] += -1 * a
                J[i_row, j0: j1] += d_arctan_D_dxy * a

                angle_d1 = calc_angle_diff(theta_b, np.arctan2(dy, dx))
                b[i_row] = calc_angle_diff(b_ik_m, angle_d1) * a
    ...
    def run_bundle_adjustment(self):
        ...
        steps_counter = 0
        while True:
            J, b = self.calc_jacobian_and_b(X_WB_e, l_xy_e)
            prog = mp.MathematicalProgram()

            dX_WB_e = prog.NewContinuousVariables(n_o + 1, dim_X, "dX_WB_e")
            ...
            prog.AddQuadraticCost(500 * ((dX_WB_e[0]) ** 2).sum())
            ...
            steps_counter += 1
            if optimal_cost / (n_o + 1) < 1e-3 or steps_counter > 500 or \
                    np.linalg.norm(dx_values) < 1e-3:
                break
        ...
        _, b = self.calc_jacobian_and_b(X_WB_e, l_xy_e)
